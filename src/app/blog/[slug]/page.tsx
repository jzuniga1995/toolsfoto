...
export async function generateMetadata({ params }: PostPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: postData.image_url ? [{ url: postData.image_url, width: 1200, height: 630, alt: postData.title }] : [],
      locale: 'es_ES',
    },
    twitter: {
    ...
    alternates: { canonical: `https://toolsfoto.com/blog/${postData.slug}` },
...
export default async function BlogPostPage({ params }: PostPageProps) {
  ...
  const articleSchema = {
    ...
    "author": { "@type": "Person", "name": postData.author || "ToolsFoto" },
    "publisher": { "@type": "Organization", "name": "ToolsFoto", "logo": { "@type": "ImageObject", "url": "https://toolsfoto.com/logo.png" } },
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Inicio", "item": "https://toolsfoto.com" },
      { "@type": "ListItem", "position": 2, "name": "Blog", "item": "https://toolsfoto.com/blog" },
      { "@type": "ListItem", "position": 3, "name": postData.title, "item": currentUrl }
    ]
  };
        ...
        <div className="bg-white border-b border-gray-200 shadow-sm">
          <div className="container mx-auto px-4 py-3 md:py-4">
            <nav className="flex items-center gap-2 text-xs sm:text-sm overflow-x-auto" aria-label="Breadcrumb">
              <Link href="/" className="text-gray-500 hover:text-blue-600 transition-colors font-medium whitespace-nowrap">Inicio</Link>
              <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400 flex-shrink-0" />
              <Link href="/blog" className="text-gray-500 hover:text-blue-600 transition-colors font-medium whitespace-nowrap">Blog</Link>
              <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400 flex-shrink-0" />
          ...
          <div className="max-w-4xl mx-auto">
            <header className="mb-8 md:mb-10">
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 md:gap-6 text-xs sm:text-sm text-gray-600 mb-8 pb-6 border-b border-gray-200">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <p className="font-bold text-gray-900 text-sm sm:text-base">{postData.author || 'Equipo ToolsFoto'}</p>
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <time dateTime={postData.published_at?.toISOString()} className="text-xs sm:text-sm">
                    {new Date(postData.published_at!).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
                  </time>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              {postData.image_url && (
                <div className="rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl mb-6 sm:mb-8 relative w-full aspect-video bg-gray-100">
                  <Image 
                    src={postData.image_url} alt={postData.title} fill className="object-cover" priority
                    sizes="(max-width: 768px) 100vw, (max-width: 1200px) 80vw, 1200px" quality={90}
                  />
                </div>
              )}
              ...
              {postData.excerpt && (
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-l-4 border-blue-600 p-4 sm:p-6 rounded-r-xl mb-6 sm:mb-8">
                  <p className="text-base sm:text-lg md:text-xl text-gray-700 leading-relaxed font-medium">{postData.excerpt}</p>
                </div>
              )}
            </header>

            {/* Content — TipTap guarda HTML, se renderiza directo */}
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 border border-blue-200 mb-8 sm:mb-12">
              <div className="flex flex-col sm:flex-row items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2">Escrito por {postData.author || 'Equipo ToolsFoto'}</h3>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <span className="px-2.5 sm:px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">Edición de Fotos</span>
                    <span className="px-2.5 sm:px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">Optimización</span>
                    <span className="px-2.5 sm:px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">Herramientas Online</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border p-6 sm:p-8 mb-8 sm:mb-12">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
                <div className="flex-1">
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">¿Te resultó útil este artículo?</h3>
                  <p className="text-sm sm:text-base text-gray-600">Compártelo con alguien a quien le pueda ayudar</p>
                </div>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl sm:rounded-2xl shadow-2xl p-6 sm:p-8 md:p-12 text-center text-white mb-8 sm:mb-12">
              ...
              <p className="text-base sm:text-xl text-blue-100 mb-6 sm:mb-8">Edita tus imágenes gratis y sin registro</p>
              <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center">
                <Link href="/compress" className="inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 bg-white text-blue-600 font-bold rounded-xl hover:bg-blue-50 transition-all shadow-lg hover:shadow-xl text-sm sm:text-base">
                  <Sparkles className="w-5 h-5" />Comprimir Imagen
                </Link>
                <Link href="/crop" className="inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 bg-green-500 text-white font-bold rounded-xl hover:bg-green-600 transition-all shadow-lg text-sm sm:text-base">
                  <Scissors className="w-5 h-5" />Recortar Imagen
                </Link>
                <Link href="/convert" className="inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 bg-purple-500 text-white font-bold rounded-xl hover:bg-purple-600 transition-all shadow-lg text-sm sm:text-base">
                  <Palette className="w-5 h-5" />Convertir Formato
                </Link>
              </div>
            </div>
            ...
            {relatedPosts.length > 0 && (
              <div className="mt-12 sm:mt-16">
                <div className="text-center mb-8 sm:mb-10">
                  <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-3 sm:mb-4">Artículos Relacionados</h2>
                  <p className="text-base sm:text-lg text-gray-600">Continúa aprendiendo sobre edición de imágenes</p>
                </div>
                      ...
                      <article className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 h-full flex flex-col border-2 border-transparent hover:border-blue-500">
                        {relatedPost.image_url ? (
                          <div className="relative h-40 sm:h-48 overflow-hidden bg-gradient-to-br from-blue-100 to-purple-100">
                            <Image src={relatedPost.image_url} alt={relatedPost.title} fill
                              className="object-cover group-hover:scale-110 transition-transform duration-500"
                              sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 400px" quality={85}
                            />
                        ...
                        <div className="p-4 sm:p-5 flex-1 flex flex-col">
                          <div className="flex items-center gap-2 sm:gap-3 text-xs text-gray-500 mb-2 sm:mb-3">
                            <div className="flex items-center gap-1">
                              ...
                              <span>{new Date(relatedPost.published_at!).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</span>
                            </div>
                            <div className="flex items-center gap-1">
                          ...
                          <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors line-clamp-2">{relatedPost.title}</h3>
